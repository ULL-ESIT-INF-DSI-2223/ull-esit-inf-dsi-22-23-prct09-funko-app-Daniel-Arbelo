...
import {FilterMapProdReduce} from "../../src/modificacionp9/FilterMapProdReduce.js";

let filterAdd = new FilterMapProdReduce([-1, 4, 6, 7, 8, 10]);
filterAdd.run();
...
describe("FilterMapProdReduce", () =>{
    ...
    it("Si se ejecuta el run sobre la ista [-1, 4, 6, 7, 8, 10] en la resultado final despues de hacer el reduce de la suma tiene que ser -70",() =>{
        expect(filterAdd.resultadoFinal).to.be.eqls(430080);
    });
});